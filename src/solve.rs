	...
	Tc: TcType + Send + Sync,
{
	...
	pub fn solve(&self) -> (f64, HashMap<&Tq, bool>, Vec<&Tc>) {
		...
		for _ in 0..self.generations {
			...
			let beta_schedule = Self::generate_beta_schedule(beta_min, beta_max, self.beta_count);
			let fut_ret = std::iter::repeat((&h, &neighbors))
				.take(self.samples)
				.collect::<Vec<_>>()
				.par_iter()
				.map(|(h, neighbors)| {
					let annealer = SimpleAnnealer::new(self.sweeps_per_beta, beta_schedule.clone());
			...
			let (energy, state) = fut_ret
				...
				.next()
				.unwrap();
			if let Some((old_energy, _, _)) = ret {
